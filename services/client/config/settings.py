...
# Канала доставки в сервисе пока НЕТ. Обвязка уведомлений готова и проверена
# (cases/notifications.py: выборка «о чём не сообщали», группировка, отметка notified_at),
# но сама доставка — заглушка, пишущая в лог. Подключается в одном месте:
# cases/notifications.deliver.
#
# EMAIL_BACKEND оставлен консольным на будущее — чтобы первое включение почты не начиналось
# с настройки SMTP, а показало письмо в логе. Ни DEFAULT_FROM_EMAIL, ни адреса сервера здесь
# нет намеренно: настройки заводятся вместе с каналом, а не заранее (ТЗ §13).
