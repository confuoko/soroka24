...
CARD_FIELDS: dict[str, tuple[str, object]] = {
    "сущность спора": ("category", _clean_or_none),
    "дата поступления": ("receipt_date", _parse_date),
    "дата принятия к производству": ("accepted_date", _parse_date),
    "статус": ("status", _clean_or_none),
}
...
def _table_with_header(tables: list[Tag], header: str) -> Tag | None:
    ...
    """
    for table in tables:
        if any(_clean(th.get_text()) == header for th in table.select("th")):
            return table
    return None
...
def _parse_card(table: Tag) -> tuple[dict, list[str]]:
    ...
    for row in _data_rows(table):
        ...
        if label == JUDGE_LABEL:
            judge = _clean_or_none(value_cell.get_text())
            if judge:
...
def _parse_sides(table: Tag) -> list[dict]:
    ...
    for row in _data_rows(table):
        ...
        if len(cells) <= SIDE_ROLE_COL:
            continue
        full_name = _clean_or_none(cells[SIDE_NAME_COL].get_text())
        if not full_name:
            # Без наименования сторона неотличима от пустой строки таблицы.
            continue
        sides.append(
            {
                "role": _clean_or_none(cells[SIDE_ROLE_COL].get_text()),
                "full_name": full_name,
            }
        )
    return sides
    ...
    def parse(self, html: str) -> dict:
        ...
        return {
            **card,
            ...
            "sides": sides,
            "events": events,
            # Историю местонахождения, судебные заседания и документы портал не
            # публикует вовсе — соответствующих блоков на странице не существует.
            "place_history": [],
            "court_sessions": [],
            "documents": [],
        }
