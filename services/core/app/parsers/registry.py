...
PARSER_BY_PAGE_TYPE: dict[str, type[CaseParser]] = {
    ...
    "B": MsudrfTypeBParser,
    # Вторая вёрстка того же движка: метки <b>, шапка таблиц без <thead>, стороны в
    # транспонированной таблице. Пермский край и Адыгея; тип выбирается по самой странице
    # (detect_page_type в app/parsers/msudrf_shared.py), а не только по домену портала.
    "C": MsudrfTypeCParser,
    # Мировые суды Санкт-Петербурга (mirsud.spb.ru) — отдельный движок, один портал на
    # все 211 судов региона.
    "D": SpbTypeDParser,
}
