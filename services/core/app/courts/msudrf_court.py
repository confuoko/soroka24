...
Сейчас резолвер отдаёт этому клиенту пять доменов — Московскую область (*.mo.msudrf.ru,
374 суда), Алтайский край (*.alt.msudrf.ru, 143 суда), Амурскую область (*.amr.msudrf.ru,
49 судов), Архангельскую область вместе с Ненецким АО (*.arh.msudrf.ru, 72 суда) и
Астраханскую область (*.ast.msudrf.ru, 53 суда), итого 691 из 6064 (см. константы доменов
ниже). Сам движок общий для 6064 судов из 71 региона (78% мировых судов страны):
разметка, капча и адреса карточек у них одинаковые, различаются только поддомены. Так что
класс писан один на все такие порталы и следующий регион подключается одной строкой в
COURT_BY_DOMAIN — но подключать его стоит, только когда разметку этого региона реально
посмотрели.
...
# Домены регионов, которые резолвер реально обслуживает. Соответствие точное: все 374 суда
# МО (код 50MS) сидят на *.mo.msudrf.ru, все 143 суда Алтайского края (код 22MS) — на
# *.alt.msudrf.ru, все 49 судов Амурской области (код 28MS) — на *.amr.msudrf.ru, все 72
# суда с кодом 29MS — на *.arh.msudrf.ru, все 53 суда Астраханской области (код 30MS) —
# на *.ast.msudrf.ru, и ни один чужой суд в эти домены не попадает.
# Список сознательно короче движка: он общий у 71 региона, но разметку мы смотрели только
# на этих пяти, и обещать остальные 5373 суда, ни разу их не открыв, нечестно. Чтобы
...
ALT_DOMAIN = "alt.msudrf.ru"
# Амурская область. Соседа-двойника, как у Алтая, здесь нет: ближайший похожий домен —
# kmr.msudrf.ru (Кемеровская область), а он отличается уже самой меткой региона.
AMR_DOMAIN = "amr.msudrf.ru"
