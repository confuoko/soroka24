...
# Липецкая область — 64 суда, код 48MS, коды сплошные. Поддомены словесные, по названию
# района: volovo, gryazi1…gryazi4, dankov1, chaplygin2.
# ВАЖНО, что домен покрывает не весь регион: на lpk.msudrf.ru сидят 62 суда, а два
# елецких (48MS0012 и 48MS0013) записаны в справочнике как
# «http://1elez@mirsud.lipetsk.ru» и «http://2elez@mirsud.lipetsk.ru» — это ДРУГОЙ
# портал, не движок msudrf. Вдобавок адрес битый: «1elez@» разбирается как имя
# пользователя, поэтому у обоих хост выходит один и тот же (mirsud.lipetsk.ru), и
# get_by_host на них честно отказывается выбирать. Так и надо: разметку того портала мы
# не смотрели. Похоже на опечатку (точку заменили собакой), но чинить справочник
# наугад нельзя — сперва надо открыть портал и посмотреть.
