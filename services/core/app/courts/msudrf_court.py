...
Сейчас резолвер отдаёт этому клиенту два региона — Московскую область (*.mo.msudrf.ru,
374 суда) и Алтайский край (*.alt.msudrf.ru, 143 суда), итого 517 из 6064 (см. MO_DOMAIN
и ALT_DOMAIN ниже). Сам движок общий для 6064 судов из 71 региона (78% мировых судов
страны): разметка, капча и адреса карточек у них одинаковые, различаются только
поддомены. Так что класс писан один на все такие порталы и следующий регион подключается
одной строкой в COURT_BY_DOMAIN — но подключать его стоит, только когда разметку этого
региона реально посмотрели.
...
# — см. MO_DOMAIN и ALT_DOMAIN ниже.
DOMAIN = "msudrf.ru"
...
# *.alt.msudrf.ru, и ни один чужой суд в эти домены не попадает.
# Список сознательно короче движка: он общий у 71 региона, но разметку мы смотрели только
# на этих двух, и обещать остальные 5547 судов, ни разу их не открыв, нечестно. Чтобы
