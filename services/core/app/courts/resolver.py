...
Внимание на Петербург: разбор его страниц (тип D) ещё НЕ НАПИСАН, а портал в
COURT_BY_DOMAIN уже стоит. Это сделано намеренно — так дела региона можно заводить
через API, доходить до портала и копить снимки страниц в S3 (снимок снимается до
разбора), а падают такие задачи уже на разборе. Для Брянска решение было обратным:
тип C доменов не получил, чтобы не тратить прокси и капчу впустую. Разница в том, что
на mirsud.spb.ru капчи нет и поход ничего не стоит.
...
COURT_BY_DOMAIN = {
    ...
    # (CourtRepository.get_by_url). Разбор типа D ещё не написан — см. докстринг модуля.
