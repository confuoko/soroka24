...
Разбор карточки (parse) пока не написан: app/parsers/spb_type_d.py — заглушка. Портал
при этом УЖЕ подключён к резолверу, в отличие от Брянска: так задачу можно дёргать
через API и копить образцы страниц в S3. Снимок снимается до разбора
(app/monitoring/tasks.py), поэтому разметка сохранится, а задача упадёт на разборе —
это ожидаемо.
"""
import logging
import re
