    ...
    * fetch_case_html_by_url — поиска нет, зато карточка открывается по прямой
    ...
    def extract_uid(self, html: str) -> str:
        ...
        if uid is None:
            ...
            #
            # Разметку прикладываем к исключению, иначе отказ неразбираем: страницу тут
            # уже не переснять (браузер закрыт), а по одному тексту ошибки не отличить
            # «дело сняли» от «портал отдал не то». url и status не знаем — их видел
            # только клиент внутри сессии.
            raise CaseNotFound(
                "На странице нет уникального идентификатора дела",
                page=PageSnapshot(html=html),
            )
        return uid
