...
from app.courts.spb_mir_court import SpbMirCourtClient, participok_from_url
...
def _fetch() -> str:
    return SpbMirCourtClient().fetch_case_html_by_url(CASE_URL)
...
def test_uid_is_extracted_from_rendered_card(portal) -> None:
    ...
    assert SpbMirCourtClient().extract_uid(_fetch()) == UID
...
def test_case_code_is_extracted_from_heading() -> None:
    ...
    assert SpbMirCourtClient().extract_case_code(RENDERED_HTML) == "2-2983/2026-98"
...
def test_missing_case_code_is_case_not_found() -> None:
    ...
    with pytest.raises(CaseNotFound):
        SpbMirCourtClient().extract_case_code("<html><body>ничего</body></html>")
