...
def test_uid_of_another_court_is_still_parsed_as_the_page_says() -> None:
    """Портал поставил на карточку УИД другого участка — разбор это не касается.

    Карточка участка № 2 Дзержинского района Ярославля (76MS0002) несёт УИД
    76MS0024-01-2026-000215-08, то есть участка № 4 Ленинского района: дело передавалось,
    а УИД присваивается при первичной регистрации и не меняется. Парсер про УИД ничего не
    знает (его достаёт клиент), и содержимое карточки от этого не страдает — проверяем, что
    страница разбирается полностью. Само расхождение кода суда с первыми 8 символами УИД
    задача пишет в лог, см. _sync_case в app/monitoring/tasks.py.
    """
    card = _parse("case_2dzr_76MS0024-01-2026-000215-08.html")

    assert card["judge_names"]
    assert len(card["events"]) == 4
    assert len(card["sides"]) == 4
