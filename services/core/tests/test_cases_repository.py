"""Тесты диффа скалярных полей дела.

Раньше upsert_by_uid молча перезаписывал поля, поэтому смена «Текущего состояния» или
...
def test_new_case_has_no_field_changes(session) -> None:
    """У нового дела дифф полей пустой: появление дела — само по себе событие."""
    case, changes = CaseRepository(session).upsert_by_uid(CASE_UID, FIRST_PARSE)
...
def test_changed_field_is_reported(session) -> None:
    """Сменился статус и появилось решение → оба поля в диффе, остальные молчат."""
    repo = CaseRepository(session)
    repo.upsert_by_uid(CASE_UID, FIRST_PARSE)
    session.flush()
    ...
    case, changes = repo.upsert_by_uid(CASE_UID, second)
...
def test_repeated_parse_gives_no_changes(session) -> None:
    """Страница не изменилась → дифф пустой. Иначе пользователь получал бы шум."""
    repo = CaseRepository(session)
    repo.upsert_by_uid(CASE_UID, FIRST_PARSE)
    session.flush()

    _, changes = repo.upsert_by_uid(CASE_UID, dict(FIRST_PARSE))

    assert changes == []


def test_value_disappearing_from_page_is_reported(session) -> None:
    ...
    case, changes = repo.upsert_by_uid(CASE_UID, moved)
...
def test_missing_key_leaves_field_untouched(session) -> None:
    ...
    case, changes = repo.upsert_by_uid(CASE_UID, {"status": "Завершено, 30.07.2026"})
