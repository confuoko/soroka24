...
DUPLICATE_ROWS = [
    ParsedPlace(place_date=date(2026, 6, 8), place_description="В канцелярии", comment=None),
    ParsedPlace(place_date=date(2026, 6, 8), place_description="В канцелярии", comment=None),
]
...
def test_same_description_different_dates_kept_separately(session, court) -> None:
    ...
    new, _, _ = PlaceHistoryRepository(session).sync_place_history(
        case,
        [
            ParsedPlace(place_date=date(2026, 7, 16), place_description="В канцелярии", comment=None),
            ParsedPlace(place_date=date(2026, 7, 21), place_description="В канцелярии", comment=None),
        ],
    )
    session.flush()
