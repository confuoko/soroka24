...
from app.parsers.parsed_case import ParsedDocument
...
PAGE_ROWS = (
    [ParsedDocument(document_date=date(2026, 7, 17), document_type="Приложение") for _ in range(21)]
    + [
        ParsedDocument(document_date=date(2026, 7, 17), document_type="Квитанция об оплате госпошлины"),
        ParsedDocument(document_date=date(2026, 7, 20), document_type="Судебный приказ"),
    ]
)
...
def test_new_attachment_added_at_the_end(session, court) -> None:
    ...
    grown = PAGE_ROWS + [
        ParsedDocument(document_date=date(2026, 7, 17), document_type="Приложение")
    ]
...
def test_other_types_do_not_shift_the_counter(session, court) -> None:
    ...
    reordered = [
        ParsedDocument(document_date=date(2026, 7, 17), document_type="Извещение")
    ] + PAGE_ROWS
