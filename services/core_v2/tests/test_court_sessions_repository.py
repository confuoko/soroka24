...
PAGE_ROWS = [
    ParsedSession(session_date=datetime(2026, 7, 30, 16, 50), place="2 - 124489, Зеленоград корп. 706", stage="Беседа", result="Проведена", basis=None),
    ParsedSession(session_date=datetime(2026, 8, 14, 10, 0), place="2 - 124489, Зеленоград корп. 706", stage="Судебное заседание", result=None, basis=None),
]
...
def test_appearing_result_is_an_update_not_a_new_session(session, court) -> None:
    ...
    held = replace(PAGE_ROWS[1], result="Отложено", basis="Неявка подсудимого")
...
def test_same_day_same_stage_different_time_are_two_sessions(session, court) -> None:
    ...
    new, _, _ = CourtSessionRepository(session).sync_court_sessions(
        case,
        [
            ParsedSession(session_date=datetime(2026, 8, 14, 10, 0), place=None, stage="Судебное заседание", result=None, basis=None),
            ParsedSession(session_date=datetime(2026, 8, 14, 15, 0), place=None, stage="Судебное заседание", result=None, basis=None),
        ],
    )
    session.flush()
...
def test_duplicate_rows_collapsed_into_one(session, court) -> None:
    ...
    new, _, _ = CourtSessionRepository(session).sync_court_sessions(
        case, [PAGE_ROWS[0], replace(PAGE_ROWS[0])]
    )
    session.flush()
