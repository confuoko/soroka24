...
DUPLICATE_EVENTS = [
    ParsedEvent(event_date=datetime(2026, 6, 8, 10, 0), state_description="Завершено", document_str=None),
    ParsedEvent(event_date=datetime(2026, 6, 8, 10, 0), state_description="Завершено", document_str=None),
]
...
def test_same_event_on_two_cards_of_one_uid(session, court) -> None:
    ...
    same_event = [ParsedEvent(event_date=datetime(2026, 6, 8, 10, 0), state_description="Завершено")]
...
def test_same_state_different_dates_kept_separately(session, court) -> None:
    ...
    new, _, _ = EventRepository(session).sync_events(
        ...
        [
            ParsedEvent(event_date=datetime(2026, 6, 8, 10, 0), state_description="Завершено", document_str=None),
            ParsedEvent(event_date=datetime(2026, 7, 8, 10, 0), state_description="Завершено", document_str=None),
        ],
    )
    session.flush()
...
def test_published_at_is_saved_and_updated_separately(session, court) -> None:
    ...
    repo = EventRepository(session)
    page = [
        ParsedEvent(event_date=datetime(2026, 6, 8, 10, 0), state_description="Завершено", document_str=None, published_at=None)
    ]
    repo.sync_events(case, page)
    session.flush()

    republished = [replace(page[0], published_at=date(2026, 6, 20))]
...
def test_published_at_stays_out_of_event_identity(session, court) -> None:
    ...
    case = _case(session, court)
    repo = EventRepository(session)
    base = ParsedEvent(event_date=datetime(2026, 6, 8, 10, 0), state_description="Завершено", document_str=None)
    repo.sync_events(case, [replace(base, published_at=date(2026, 6, 10))])
    session.flush()
    first_uid = case.events[0].uid

    repo.sync_events(case, [replace(base, published_at=date(2026, 6, 20))])
    session.flush()
