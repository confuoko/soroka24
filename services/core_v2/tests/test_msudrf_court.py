...
from app.courts import msudrf as msudrf_court
...
def _fetch(client=None) -> tuple[str, MsudrfClient]:
    """Сходить за карточкой и отдать её РАЗМЕТКУ (клиент теперь возвращает FetchedCard)."""
    client = client or MsudrfClient()
    return client.fetch_card_by_url(CASE_URL).html, client
...
def test_client_goes_through_the_proxy_it_was_given(portal) -> None:
    ...
    _fetch(MsudrfClient(proxy=FROM_POOL))
...
def test_each_captcha_is_reported_for_accounting(portal) -> None:
    ...
    html, _ = _fetch(MsudrfClient(on_captcha_attempt=reported.append))
...
# --------------------------------------------------------------------- УИД со страницы
def test_uid_is_found_on_the_card() -> None:
    """УИД ищет обычная функция, одна на все порталы: формат общероссийский."""
    assert find_uid(CARD_HTML) == UID
    assert UID[:8] == "50MS0095"


def test_missing_uid_is_none_not_an_error() -> None:
    """Нет УИД на странице — это None, а не исключение.

    В старом core клиент бросал бы здесь CaseNotFound, но в бою этой ветки не
    существовало: у карточек без УИД ключ считается от адреса (synthetic_uid), и решает
    это разрешение identity, а не поиск по тексту. Функция лишь честно отвечает
    «не нашла».
    """
    assert find_uid(EMPTY_HTML) is None
...
def test_case_code_is_extracted_from_heading() -> None:
    ...
    assert extract_msudrf_case_code(html) == "2-1244/2026"
...
def test_material_case_code_starts_with_cyrillic_letter() -> None:
    ...
    assert extract_msudrf_case_code(html) == "М-2987/2026"
...
def test_material_number_is_a_case_code_too() -> None:
    ...
    assert extract_msudrf_case_code(html) == "13-163/2026"
...
def test_missing_case_code_is_case_not_found() -> None:
    ...
    with pytest.raises(CaseNotFound):
        extract_msudrf_case_code(EMPTY_HTML)
