"""Правила identity, прибитые к конкретным значениям.
...
**Это дословная копия services/characterization/test_identity_golden.py**, снятого со
старого core до переноса, с изменёнными только импортами. В том, что здесь ничего не
пришлось подгонять, и состоит доказательство: те же прибитые значения проходят на новом
коде, значит identity перенос пережила. Если при правках придётся поменять хоть один
литерал — у всех существующих дел поедут uid дочерних строк.
