...
class SpbClient(CourtClient):
    ...
    def fetch_card_by_url(self, url: str) -> FetchedCard:
        ...
        Номер участка достаём из самого адреса: на этом портале он стоит в пути
        (/cases/detail/98/), и по нему определяется суд. Номер ДЕЛА здесь не трогаем —
        он лежит в тексте страницы, и его достаёт extract_spb_case_code.
        """
        with ChromiumSession(headless=self._headless, proxy=self._proxy) as session:
            ...
            try:
                ...
                return FetchedCard(
                    ...
                    participok_no=participok_from_url(url),
                    source_url=url,
                    status=status,
                )
            except CourtError:
...
def extract_spb_case_code(html: str) -> str:
    """Достать номер дела из заголовка страницы: «Судебное дело №2-2983/2026-98».

    Обычная функция, а не метод клиента: номер дела — содержимое СТРАНИЦЫ, а клиент
    отвечает за то, как до страницы добраться. Разметка заголовка у порталов разная,
    поэтому функция портальная, но к способу похода она отношения не имеет.

    Номер входит в ключ карточки и в имя папки снимка в S3, так что без него дело не
    сохранить. Отсутствие заголовка означает, что открылась не карточка, — повторять
    поход бессмысленно, отсюда CaseNotFound.
    """
