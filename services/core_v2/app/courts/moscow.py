...
class MoscowClient(CourtClient):
    """Клиент мировых судов Москвы (mos-sud.ru) — единственный портал с поиском по УИД.

    Вёрстку страницы здесь не объявляем: клиент про неё не знает и знать не должен.
    """
            ...
            try:
                ...
                for index in range(found):
                    ...
                    cards.append(
                        FetchedCard(
                            html=html,
                            # Номер дела и участок взяты ИЗ ТАБЛИЦЫ результатов, а не
                            # из карточки: там номер приходит под разными метками и
                            # потому не всегда, а суда нет вовсе.
                            case_code=code,
                            participok_no=participok_no,
                            status=card_status,
                        )
                    )
