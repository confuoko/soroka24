...
def _participok_in_path_rule(host: Optional[str]):
    ...
    Знание про номер участка в пути живёт рядом с остальным знанием об этом портале
    (там же лежит и разбор ссылки), а не расползается по репозиторию.

    В старом core импорт клиента приходилось делать ВНУТРИ функции: на уровне модуля
    получался цикл — app.courts.spb_mir_court тянул app.browser, тот app.browser.proxy,
    а тот app.repositories, то есть этот самый пакет. Цикла больше нет: аренда прокси
    переехала из app/browser в app/services/proxy_pool.py, и пакет браузера про БД
    ничего не знает. Поэтому импорт здесь обычный, на уровне модуля.
    """
    rules = {
        spb.DOMAIN: (spb.REGION_CODE, spb.participok_from_url),
    }
