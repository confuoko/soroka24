...
last_checked_at и last_changed_at — факты о деле: «когда мы последний раз смотрели
страницу» и «когда на ней последний раз что-то менялось». Показывать их надо обе и рядом:
«дело не менялось три месяца» и «дело три месяца не проверяли» — разные вещи, и вторая
означает, что что-то сломалось.

is_on_monitoring — единственное поле, которым распоряжается не core: его выставляет
клиентский сервис запросом PUT /monitoring/cases, потому что только он знает, за какими
делами следят пользователи. Что делать с этим флагом, решает core (см. app/tasks.py,
...
class Case(Base):
    ...
    __table_args__ = (
        ...
        # Частичный индекс под единственный запрос планировщика: «дела на мониторинге,
        # самые давно не проверявшиеся первыми».
        #
        # Частичный, а не обычный: на мониторинге заведомо меньшинство карточек (дела
        # копятся навсегда, подписки — нет), и индексировать в нём многомиллионное
        # большинство со значением false незачем.
        #
        # Колонка last_checked_at, а не id: планировщик по ней и фильтрует, и сортирует
        # (см. CaseRepository.list_monitored_ids), так что индекс закрывает запрос целиком.
        Index(
            "ix_case_on_monitoring",
            "last_checked_at",
            postgresql_where=text("is_on_monitoring"),
        ),
    )
    ...
    # таблице результатов поиска либо из хоста ссылки (см. app/services/discovery.py).
    ...
    # Дело обходится регулярно, потому что на него подписан хотя бы один пользователь.
    #
    # Единственное поле карточки, которое core себе НЕ выставляет: кто на что подписан,
    # знает клиентский сервис, и он же приводит флаг в соответствие запросом
    # PUT /monitoring/cases (см. app/api/monitoring.py). Core решает лишь, что с этим
    # флагом делать: раз в сутки набирает помеченные дела и ставит их на обычный
    # повторный обход.
    #
    # Флаг ОДИН на карточку, а не на подписку, и это важно: на дело с тремя подписчиками
    # мы всё равно ходим один раз. Поход на портал стоит прокси и оплаченной капчи,
    # поэтому он не должен множиться на число заинтересованных.
    is_on_monitoring: Mapped[bool] = mapped_column(
