...
from app.parsers.text import clean, clean_or_none, parse_date
...
def _cell_or_none(cells: list, index: int) -> str | None:
    """Значение ячейки по индексу через clean_or_none; нет такой колонки → None.

    Нужен для хвостовых колонок таблицы заседаний: их может не оказаться в разметке,
    и падать из-за этого нельзя.
    """
    return clean_or_none(cells[index].get_text()) if index < len(cells) else None
...
CARD_FIELDS = (
    (re.compile(r"^[НнHh]омер заявления\b"), "application_number", clean_or_none),
    (
    ...
    (re.compile(r"^[ДдDd]ата поступления\b"), "receipt_date", parse_date),
    (re.compile(r"^[ДдDd]ата регистрации\b"), "registration_date", parse_date),
    (
    ...
    (re.compile(r"^[КкKk]атегория дела\b"), "category", clean_or_none),
    (re.compile(r"^[ТтTt]екущее состояние\b"), "status", clean_or_none),
)
...
def _parse_state_history(soup: BeautifulSoup) -> list[dict]:
    ...
    """
    heading = soup.find(
        "h3", string=lambda s: s is not None and clean(s) == STATE_HISTORY_HEADING
    )
    if heading is None:
        return []
    ...
    for row in table.select("tbody tr"):
        ...
        state_description = clean(cells[1].get_text())
        document_str = clean(cells[2].get_text()) or None

        # Дата + описание обязательны — иначе событие не может участвовать в детекте.
        if event_date is None or not state_description:
            continue

        # published_at здесь не выставляется: колонки «Дата размещения» на карточке
        # мировых судов Москвы нет вовсе. В ParsedEvent поле останется None — на запись
        # это не влияет, оно не участвует ни в identity события, ни в сверке полей дела.
        events.append(
            ParsedEvent(
...
def _parse_place_history(soup: BeautifulSoup) -> list[dict]:
    ...
    """
    heading = soup.find(
        "h3", string=lambda s: s is not None and clean(s) == PLACE_HISTORY_HEADING
    )
    if heading is None:
        return []
    table = heading.find_next("table")
    ...
    for row in table.select("tbody tr"):
        ...
        place_date = parse_date(cells[0].get_text())
        place_description = clean(cells[1].get_text())
        comment = clean(cells[2].get_text()) or None
    ...
    for row in box.select("table tbody tr"):
        ...
        sessions.append(
            ParsedSession(
                ...
                place=clean_or_none(cells[1].get_text()),
                stage=stage,
...
def _documents_table(box: Tag) -> Tag | None:
    ...
    for table in box.find_all("table"):
        headings = [clean(th.get_text()) for th in table.select("th")]
...
def _parse_documents(soup: BeautifulSoup) -> list[dict]:
    ...
    for row in table.select("tbody tr"):
        ...
        document_date = parse_date(cells[0].get_text())
        document_type = clean(cells[1].get_text())
        ...
        documents.append(
            ParsedDocument(document_date=document_date, document_type=document_type)
        )
    return documents
...
def _parse_participants(role: str, value_el: Tag) -> list[dict]:
    ...
    for nodes in _split_by_br(value_el):
        ...
        if full_name is None:
            full_name = clean(PARTICIPANT_ARTICLE_RE.sub("", _chunk_text(nodes)))
        if full_name:
            participants.append(ParsedSide(role=role, full_name=full_name))
    return participants
...
def _parse_sides(value_el: Tag) -> list[dict]:
    ...
    for strong in value_el.find_all("strong"):
        role = clean(strong.get_text()).rstrip(":").strip()
...
class MoscowTypeAParser(CaseParser):
    ...
    def parse(self, html: str) -> dict:
        ...
        for row in soup.select(ROW_SELECTOR):
            ...
            # Метку сверяем только после clean: значения и метки обложены пробелами
        ...
        # засеяны из CARD_FIELDS в начале parse. Единственное скалярное поле, которого
        # у Москвы нет, — «Дата принятия к производству» (её отдаёт только Петербург);
        # в ParsedCase оно остаётся UNSET, и колонка в БД не трогается.
        return ParsedCase(
            **card,
